...
do
  ...
  do
    ...
    echo x   Run = $run
    echo x   "./ns3 run \"$PROGRAM_NAME --routingProtocol=$ROUTING"
    ...
    echo "Start time:"
    date
    ./ns3 run "$PROGRAM_NAME --routingProtocol=$ROUTING --nNodes=$n --nSources=$N_SOURCE_NODES --dataRate=$RATE --packetSize=$PACKET_SIZE --startRngRun=$START --currentRngRun=$run --stopRngRun=$STOP --scenario=$SCENARIO" 
    echo "StopTime:"
    date
    echo --------------------------------------------------------------------------
